...
void plan_buffer_line(float x, float y, float z, float feed_rate, uint8_t invert_feed_rate) 
{
  ...
  int32_t target[3];
  target[X_AXIS] = mylround(x*settings.steps_per_mm[X_AXIS]);
  target[Y_AXIS] = mylround(y*settings.steps_per_mm[Y_AXIS]);
  target[Z_AXIS] = mylround(z*settings.steps_per_mm[Z_AXIS]);     
