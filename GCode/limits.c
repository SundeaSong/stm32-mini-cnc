...
static void homing_cycle(uint8_t cycle_mask, int8_t pos_dir, bool invert_pin, float homing_rate) 
{
  ...
  if (!pos_dir) {
	out_bits0 ^= DIRECTION_MASK; 
	}   // Invert bits, if negative dir.
  _delay_us(settings.pulse_microseconds);
  // Initialize stepping variables

//	counter_x = -(step_event_count >> 1);//原
...
//  trap_counter = MICROSECONDS_PER_ACCELERATION_TICK/2; // Acceleration trapezoid counter
  for(;;) {
    ...
    if (invert_pin) { 
		...
		} // If leaving switch, invert to move.
...
//      }
    ...
    if (!(cycle_mask) || (sys.execute & EXEC_RESET)) { return; }
		...
		OutputControl((STEPPING_PORT & ~STEPPING_MASK) | out_bits );//	
    _delay_us(settings.pulse_microseconds);
		OutputControl((STEPPING_PORT & ~STEPPING_MASK) | 0);
    _delay_us(500); //step_delay
...
{ 
  ...
  homing_cycle(HOMING_SEARCH_CYCLE_0, true, false, settings.homing_seek_rate);  // Search cycle 0 z
	...
	homing_cycle(HOMING_SEARCH_CYCLE_1, true, false, settings.homing_seek_rate);  // Search cycle 1 x y
	
	_delay_ms(settings.homing_debounce_delay); 
	
	homing_cycle(HOMING_LOCATE_CYCLE, false, true, settings.homing_seek_rate);  // Search cycle  xyz
	
	 _delay_ms(settings.homing_debounce_delay); 
	
	homing_cycle(HOMING_LOCATE_CYCLE, true, false, settings.homing_seek_rate);  // Search cycle  xyz
