  ...
  if ((TIM2->SR & 0x0001) != 0)                  // check interrupt source
    ...
    TIM2->SR &= ~(1<<0);                          // clear UIF flag
		...
		//	TIM4->CR1 |= (1<<0); 
			...
			if (current_block == NULL) {// Anything in the buffer? If so, initialize next motion.
				current_block = plan_get_current_block();
				if (current_block != NULL) {
					if (sys.state == STATE_CYCLE) {// During feed hold, do not update rate and trap counter. Keep decelerating.
